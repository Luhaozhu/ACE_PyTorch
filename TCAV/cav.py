...
class CAV:  # CAV类相当于就是把定义的concept进行保存，保存成一个pkl文件
    r"""
    ...
    def __init__(
        ...
        and loads them from the disk (storage).这个类封装了CAVs对象的实例，将它们保存在 
        并从磁盘(存储)加载它们。
    ...
    def assemble_save_path(
        ...
        if not os.path.exists(os.path.join(path,model_id)):
            os.mkdir(os.path.join(path,model_id))
    ...
    def save(self):
        ...
        save_dict = {
            "concept_ids": [c.id for c in self.concepts],  # concept的索引号
            "concept_names": [c.name for c in self.concepts], # concept的类别
            "layer": self.layer,   # 要获取activation的层
            "stats": self.stats,
        }
    ...
    def load(cavs_path: str, model_id: str, concepts: List[Concept], layer: str):
        ...
        if os.path.exists(cavs_path):
            ...
            concept_names = save_dict["concept_names"]  # 导入名称
            concept_ids = save_dict["concept_ids"]  # 导入概念的索引号
            concepts = [
